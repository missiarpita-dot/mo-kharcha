...
function getMonthSummaryFromDb(db, monthId) {
  ...
  if (currentIndex > 0) {
    ...
    const prevSummary = getMonthSummaryFromDb(db, prevMonth.id);
...
async function createWorkbookFromDb() {
  ...
  const summaries = sortedMonths.map((m) => getMonthSummaryFromDb(db, m.id));
...
async function syncDesktopExcel() {
  try {
    const userProfile = process.env.USERPROFILE || 'C:\\Users\\a_sah';
    const desktopPaths = [
      path.join(userProfile, 'OneDrive', 'Desktop', 'Household_Expense.xlsx'),
      path.join(userProfile, 'Desktop', 'Household_Expense.xlsx')
    ];

    const workbook = await createWorkbookFromDb();

    for (const desktopPath of desktopPaths) {
      const targetDir = path.dirname(desktopPath);
      if (fs.existsSync(targetDir)) {
        await workbook.xlsx.writeFile(desktopPath);
        console.log(`✅ Desktop Excel auto-updated at: ${desktopPath}`);
      }
    }
  } catch (err) {
    console.error('⚠️ Could not auto-update Desktop Excel (file may be open in MS Excel):', err.message);
  }
}

module.exports = {
  createWorkbookFromDb,
  syncDesktopExcel
};
