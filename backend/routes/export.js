...
const SECTION_BG    = 'FFDAE3F3';
const TOTAL_BG      = 'FFFFF2CC';
const DUE_COLOR     = 'FFC00000';
const SETTLED_COLOR = '00B050';
const INR_FMT       = '₹#,##0.00';
...
function buildOverviewSheet(workbook, summaries) {
  ...
  ws.columns = [{ width: 20 }, { width: 18 }, { width: 20 }, { width: 20 }, { width: 18 }, { width: 24 }];
  ...
  ws.mergeCells('A2:F2');
  ws.getCell('A2').value = "Pulls totals automatically from each month's sheet.";
  ws.getCell('A2').font  = { italic: true, color: { argb: 'FF595959' }, size: 10 };
  ws.getRow(2).height = 24;
  ws.getRow(3).height = 8;

  ['Month', 'Opening Due (₹)', 'Total Expenses (₹)', 'Total Received (₹)', 'Closing Due (₹)', 'Status']
    .forEach((h, i) => styleHeaderCell(ws.getRow(4).getCell(i + 1), h));
  ws.getRow(4).height = 22;

  let cumExpenses = 0, cumReceived = 0;
  summaries.forEach((s, idx) => {
    ...
    [2, 3, 4, 5].forEach(c => { row.getCell(c).numFmt = INR_FMT; });
    const sc = s.status === 'Settled' ? SETTLED_COLOR : s.status === 'Due from Father' ? DUE_COLOR : '7030A0';
    row.getCell(6).font = { bold: true, color: { argb: 'FF' + sc } };
    if (idx % 2 === 1) [1,2,3,4,5,6].forEach(c => { row.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF2F7FF' } }; });
    [1,2,3,4,5,6].forEach(c => applyBorder(row.getCell(c)));
    cumExpenses += s.totalExpenses;
    cumReceived += s.totalReceived;
  });

  const cumRow = ws.getRow(5 + summaries.length + 1);
  cumRow.height = 22;
...
function buildMonthSheet(workbook, summary, expenses, payments) {
  ...
  ws.columns = [{ width: 14 }, { width: 18 }, { width: 32 }, { width: 14 }, { width: 18 }, { width: 22 }];
  ...
  ws.mergeCells('A2:F2');
  ws.getCell('A2').value = 'Enter your data in the shaded cells. All totals and balances update automatically.';
  ws.getCell('A2').font  = { italic: true, color: { argb: 'FF595959' }, size: 10 };
  ...
  ['Date', 'Category', 'Description', 'Amount (₹)', 'Paid From', 'Status']
    .forEach((h, i) => styleHeaderCell(ws.getRow(6).getCell(i + 1), h));
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (exp) {
      ...
      row.getCell(4).value = exp.amount; row.getCell(4).numFmt = INR_FMT;
      row.getCell(5).value = exp.paid_from;
      row.getCell(6).value = exp.status;
      const sc = exp.status === 'Settled' ? SETTLED_COLOR : DUE_COLOR;
      row.getCell(6).font = { color: { argb: 'FF' + sc }, bold: true };
      if (exp.paid_from === 'Own Money') [1,2,3,4,5,6].forEach(c => { row.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF8F0' } }; });
    }
    [1,2,3,4,5,6].forEach(c => applyBorder(row.getCell(c)));
  }
  ...
  totExpRow.height = 22;
  totExpRow.getCell(3).value = 'Total Expenses (this month)'; totExpRow.getCell(3).font = { bold: true }; totExpRow.getCell(3).alignment = { horizontal: 'right' };
  totExpRow.getCell(4).value = summary.totalExpenses; totExpRow.getCell(4).numFmt = INR_FMT; totExpRow.getCell(4).font = { bold: true };
  [3,4].forEach(c => { totExpRow.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: TOTAL_BG } }; applyBorder(totExpRow.getCell(c)); });
  ...
  ['Date', 'Amount Received (₹)', 'Note'].forEach((h, i) => styleHeaderCell(ws.getRow(50).getCell(i + 1), h));
  ws.getRow(50).height = 20;

  for (let i = 0; i < 15; i++) {
    const row = ws.getRow(51 + i);
    row.height = 18;
    const pay = payments[i];
    if (pay) { row.getCell(1).value = pay.date; row.getCell(2).value = pay.amount; row.getCell(2).numFmt = INR_FMT; row.getCell(3).value = pay.note; }
    [1,2,3].forEach(c => applyBorder(row.getCell(c)));
  }

  const totRecRow = ws.getRow(66);
  totRecRow.height = 22;
  totRecRow.getCell(1).value = 'Total Received (this month)'; totRecRow.getCell(1).font = { bold: true };
  totRecRow.getCell(2).value = summary.totalReceived; totRecRow.getCell(2).numFmt = INR_FMT; totRecRow.getCell(2).font = { bold: true };
  [1,2].forEach(c => { totRecRow.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: TOTAL_BG } }; applyBorder(totRecRow.getCell(c)); });
  ...
  [
    ...
    row.height = 20;
    row.getCell(1).value = label; row.getCell(1).font = { bold: i === 4 };
    row.getCell(3).value = value; row.getCell(3).numFmt = INR_FMT; row.getCell(3).font = { bold: i === 4 };
    [1,3].forEach(c => applyBorder(row.getCell(c)));
  });

  const statusRow = ws.getRow(74);
  statusRow.height = 22;
  statusRow.getCell(1).value = 'Status'; statusRow.getCell(1).font = { bold: true };
  statusRow.getCell(3).value = summary.status;
  const sc = summary.status === 'Settled' ? SETTLED_COLOR : summary.status === 'Due from Father' ? DUE_COLOR : '7030A0';
  statusRow.getCell(3).font = { bold: true, color: { argb: 'FF' + sc } };
...
router.get('/', async (req, res) => {
  try {
    ...
    buildOverviewSheet(workbook, valid);

    for (const summary of valid) {
      const expenses = await db.getExpensesByMonth(summary.id);
      const payments = await db.getPaymentsByMonth(summary.id);
      buildMonthSheet(workbook, summary, expenses, payments);
    }

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', 'attachment; filename="Household_Expense.xlsx"');
